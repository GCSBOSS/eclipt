...
function getGlobalHelp(cli){
    //let hasOpts = Object.keys(cli.options).length > 0; TODO consider making help and version optional.
    let hasCmds = Object.keys(cli.commands).length > 0;
    let output = `\r\nUsage: ${cli.settings.name} `;

    //if(hasOpts) TODO consider making help and version optional.
    output += '[OPTIONS] ';
...
function getCommandHelp(name, cmd, cli){
    //let hasOpts = Object.keys(cmd.options).length > 0; TODO consider making help and version optional.
