...
\o                                                                  */
function getCommandsHelp(cli){
    let output = '\r\nCommands:\r\n';
    let cmdArr = [];
    let nameMaxSize = 0;

    for(let name in cli.commands){
        cli.getCommand(name);
        nameMaxSize = Math.max(name.length, nameMaxSize);
        cmdArr.push({ summary: cli.commands[name].summary || ' ', name: name });
    }
...
function getGlobalHelp(cli){
    ...
    let argStr = Array.isArray(cli.settings.expectedArgs)
        ? cli.settings.expectedArgs.join(' ') : '[ARGS]';

    output += compose(!cli.settings.noArgs, '[--] ' + argStr) + '\r\n';

    output += compose(cli.settings.summary, `\r\n${cli.settings.summary}\r\n`);

    //if(hasOpts) TODO consider making help optional.
    output += getOptionsHelp(cli.options);

    if(hasCmds)
        output += getCommandsHelp(cli) +
...
function getCommandHelp(name, cmd, cli){
    ...
    let argStr = Array.isArray(cmd.expectedArgs)
        ? cmd.expectedArgs.join(' ') : '[ARGS]';

    output += compose(!cmd.noArgs, '[--] ' + argStr) + '\r\n';
