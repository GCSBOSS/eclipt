...
function buildData(tokens){
    ...
    for(let token of tokens){
        ...
        //console.log(tokens[token.prev]);
        //console.log('\n\n');

        let justHadOptyOrFirst = token.first || tokens[token.prev].type in { opt: true, flag: true };
        ...
        if(token.type == 'aa' && justHadOptyOrFirst)
            phase++;
        ...
        if(phase == 2){
            ...
            token.type = !context.options[token.data][2] ? 'flag' : 'opty';

            let def = context.options[token.data][3];

            data[token.data] = token.type == 'opty' ? def : true;

            if(token.type == 'opty' && !def && (token.last || tokens[token.next].type !== 'aa'))
                throw new Error('Missing required value for option: ' + token.data);

            token.key = token.data;
        }
        else if(token.type == 'aa' && tokens[token.prev].type == 'opty'){
            data[tokens[token.prev].key] = token.data;
            token.type = 'opt';
        }
    }
...
module.exports = class CLI {
    ...
    execute(input){
        ...
        let data = new CLIInputData(this, args);
